...
test("server-renders the finished portfolio", async () => {
  ...
  assert.match(html, /Selected track record · Before HireData/);
  assert.match(html, /Built for a national audience\./);
  assert.match(html, /A three-initiative exit into a leading Dutch insurance group\./);
  assert.match(html, /Weekly co-host on Dutch national business radio\./);
  assert.match(html, /How we built national reach · 2014—2015/);
  assert.match(html, /BNR ZZP Café/);
  assert.match(html, /Diana Matroos/);
  assert.match(html, /then part of leading Dutch insurer a\.s\.r\./);
  ...
  assert.match(html, /Prime Minister of the Netherlands/);
  ...
  assert.match(html, /i\.ytimg\.com\/vi\/mynvKKeugcs\/maxresdefault\.jpg/i);
  assert.doesNotMatch(html, /Watch the interview on YouTube/);
